...
class HQMFVsSimpleTest < Minitest::Test
  ...
  def remap_populations(simple_xml_model, hqmf_model)
    ...
    hqmf_model.instance_variable_get(:@populations).map! { |pop| pop.reject { |key, _vaule| key == 'stratification' } }

    # population titles in HQMF2 can be ignored
    hqmf_model.instance_variable_get(:@populations).map! { |pop| pop.reject { |key, _vaule| key == 'title' } }
    simple_xml_model.instance_variable_get(:@populations).map! { |pop| pop.reject { |key, _vaule| key == 'title' } }
